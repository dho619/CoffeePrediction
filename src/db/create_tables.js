...
export default async function create_tables_if_not_exists() {
    ...
    if (new_version) {
        ...
        try {
            ...
            await execute_db_offline("CREATE TABLE classifications (replication_sequence  INTEGER PRIMARY KEY, id text not null, name text, description text, image text, area_id text, area_name text, user_id text, type_action text);");
